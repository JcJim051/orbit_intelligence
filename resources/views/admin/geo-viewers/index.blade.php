    ...
    <div class="grid gap-6 xl:grid-cols-2">
        ...
        <details class="panel action-disclosure" @if($errors->any()) open @endif>
            ...
            <form method="post" action="{{ route('admin.geo-layers.store') }}" class="mt-5 grid gap-4 sm:grid-cols-2" data-slug-suggestion>
                ...
                <label class="field sm:col-span-2"><span>URL del servicio</span><input type="url" name="source_url" required maxlength="2000" placeholder="https://.../ows o https://.../capa.geojson"></label>
                ...
                <label class="field"><span>Acceso para la comunidad</span><select name="access_policy" required>@foreach(\App\Enums\GeoLayerAccessPolicy::cases() as $policy)<option value="{{ $policy->value }}" @selected($policy === \App\Enums\GeoLayerAccessPolicy::Pending)>{{ $policy->label() }}</option>@endforeach</select><small>Debe definirse antes de publicar el visor.</small></label>
                ...
                <label class="field"><span>Color de línea</span><input type="color" name="color" value="#4338ca" required></label>
                <label class="field"><span>Color de relleno</span><input type="color" name="fill_color" value="#818cf8" required></label>
            ...
            @foreach($layers as $layer)
                <details class="panel" id="layer-{{ $layer->slug }}">
                    ...
                    <form method="post" action="{{ route('admin.geo-layers.update', $layer) }}" class="mt-5 grid gap-4 sm:grid-cols-2">
                        ...
                        <label class="field sm:col-span-2"><span>URL del servicio</span><input type="url" name="source_url" value="{{ $layer->source_url }}" required></label>
                        ...
                        <label class="field"><span>Color de línea</span><input type="color" name="color" value="{{ data_get($layer->style, 'color', '#4338ca') }}"></label>
                        <label class="field"><span>Relleno</span><input type="color" name="fill_color" value="{{ data_get($layer->style, 'fillColor', '#818cf8') }}"></label>
