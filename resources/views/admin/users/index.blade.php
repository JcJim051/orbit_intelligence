...
<div class="space-y-6">
    ...
    @if(auth()->user()->isAdmin())<details class="panel action-disclosure" @if($errors->any()) open @endif>
            ...
            <div class="divide-y divide-slate-100">
                @foreach($users as $user)
                    <form method="post" action="{{ route('admin.users.update', $user) }}" class="grid gap-4 p-5 md:grid-cols-[minmax(180px,1fr)_minmax(210px,280px)_130px_auto] md:items-center">@csrf @method('PATCH')
                        ...
                        @php $assignableRoles = auth()->user()->isAdmin() ? \App\Enums\UserRole::cases() : [\App\Enums\UserRole::Member, \App\Enums\UserRole::SiidManager]; $managerCanEdit = auth()->user()->isAdmin() || in_array($user->role, [\App\Enums\UserRole::Member, \App\Enums\UserRole::SiidManager], true); @endphp
                        <label class="field"><span class="md:sr-only">Rol</span><select name="role" aria-label="Rol de {{ $user->name }}" @disabled($user->is(auth()->user()) || !$managerCanEdit)>@foreach($assignableRoles as $role)<option value="{{ $role->value }}" @selected($user->role === $role)>{{ $role->label() }}</option>@endforeach</select></label>
                        ...
                        <button class="btn-secondary" @disabled($user->is(auth()->user()) || !$managerCanEdit)>{{ $user->is(auth()->user()) || !$managerCanEdit ? 'Protegida' : 'Guardar' }}</button>
                    </form>
                @endforeach
            </div>
        </div>
